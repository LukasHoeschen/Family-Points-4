...
public struct RSAKeyPair {
    ...
    // Function to save the private key to Keychain
    public func savePrivateKeyToKeychain(identifier: String) -> Bool {
        guard let privateKeyData = privateKey.exportToData(),
              let publicKeyData = publicKey.exportToData() else {
            return false
        }

        let keychainIdentifier = identifier + ".private"
        let savePrivateKeySuccess = Keychain.save(data: privateKeyData, forKey: keychainIdentifier)

        let keychainPublicIdentifier = identifier + ".public"
        let savePublicKeySuccess = Keychain.save(data: publicKeyData, forKey: keychainPublicIdentifier)

        return savePrivateKeySuccess && savePublicKeySuccess
    }

    // Function to load the private key from Keychain
    public static func loadKeyPairFromKeychain(identifier: String) -> RSAKeyPair? {
        let keychainIdentifier = identifier + ".private"
        guard let privateKeyData = Keychain.load(forKey: keychainIdentifier),
              let privateKey = SecKey.loadFromData(privateKeyData) else {
            return nil
        }

        let keychainPublicIdentifier = identifier + ".public"
        guard let publicKeyData = Keychain.load(forKey: keychainPublicIdentifier),
              let publicKey = SecKey.loadFromData(publicKeyData) else {
            return nil
        }
...
class Keychain {
    class func save(data: Data, forKey key: String) -> Bool {
    ...
    class func load(forKey key: String) -> Data? {
        let query: [String: Any] = [
            kSecClass as String: kSecClassGenericPassword,
            kSecAttrAccount as String: key,
...
public struct RSAPrivateKey {
    ...
    #if DEBUG
    public var __debug_publicKey: SecKey { self.privateKey }
    #endif
    ...
    /// Allows you to load an RSA public key (i.e. one downloaded from the net).
    ///
    public static func load(rsaPrivateKeyData: Data) -> RSAPrivateKey? {
        if let privateKey: SecKey = .loadFromData(rsaPrivateKeyData) {
